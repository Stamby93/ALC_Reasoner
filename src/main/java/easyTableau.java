...
public class easyTableau implements Tableau{
    ...
    protected easyTableau(OWLClassExpression concept, int parent) {
        ...
        this.parent = parent;
        conceptComparator = new Comparator<OWLClassExpression>() {
            @Override
            public int compare(OWLClassExpression expression, OWLClassExpression t1) {
        ...
        LoggerManager.writeDebugLog("SAT: "+ parent, easyTableau.class);
    ...
    public boolean SAT() {
        ...
        if(isWorking()) {
            ...
            switch (type) {
                ...
                case OWL_CLASS:
                case OBJECT_COMPLEMENT_OF:
                    LoggerManager.writeDebugLog("Rule: "+ workingRule + " CLASS :" + OntologyRenderer.render(conceptList.get(workingRule)), easyTableau.class);
                    if (checkClash()) {
    ...
    private boolean applyIntersection(){
        LoggerManager.writeDebugLog("Rule: " + workingRule + " INTERSECTION: "+ OntologyRenderer.render(conceptList.get(workingRule)), easyTableau.class);
        ...
        operand.sort(conceptComparator);
        for (int i = 0; i < operand.size(); i++) {
            if(!conceptList.contains(operand.get(i)))
                conceptList.add(conceptList.size(),operand.get(i));
        }
        workingRule ++;
        if (SAT())
            return true;
    ...
    private boolean applyUnion(){
        LoggerManager.writeDebugLog("Rule: "+ workingRule + " UNION: " + OntologyRenderer.render(conceptList.get(workingRule)), easyTableau.class);

        Integer rule = workingRule;
        ...
        for (int i = 0; i < jointedList.size(); i++) {
            ...
            if (!conceptList.contains(jointedList.get(i))) {
                LoggerManager.writeDebugLog("CHOICE " + OntologyRenderer.render(jointedList.get(i)), ChronologicalTableau.class);

                conceptList.add(conceptList.size(), jointedList.get(i));
    ...
    private boolean applySome(OWLClassExpression rule){
        LoggerManager.writeDebugLog("Rule: " + workingRule + " SOME: " + OntologyRenderer.render(rule), easyTableau.class);
        ...
        if (related.size()!=0) {
            ...
            for (Integer r : related) {
                ...
                if(filler.equals(flag.getFiller())){
                    LoggerManager.writeDebugLog("SOME ALREADY PRESENT", easyTableau.class);
        ...
        direct = new easyTableau(filler, workingRule);
        if(direct.SAT()) {
            LoggerManager.writeDebugLog("SOME "+workingRule+" SATISFIABLE", easyTableau.class);
            ...
            LoggerManager.writeDebugLog("SOME UNSATISFIABLE", easyTableau.class);
    ...
    private boolean applyAll(OWLClassExpression rule){
        LoggerManager.writeDebugLog("Rule: " + workingRule + " ALL: "+ OntologyRenderer.render(rule), easyTableau.class);
        ...
        if (someRelation.get(oe) == null){
            LoggerManager.writeDebugLog("ALL NO CONDITIONS", easyTableau.class);
            ...
            for (int i  = 0 ; i < related.size(); i++){
                ...
                flag = (OWLObjectSomeValuesFrom) conceptList.get(related.get(i));
                ...
                if(!filler.equals(flag.getFiller())){
                    ...
                    Tableau Tflag = new easyTableau(concept.getNNF(),workingRule);

                    if(!Tflag.SAT()){
                        LoggerManager.writeDebugLog("ALL UNSATISFIABLE", easyTableau.class);
        ...
        for (int i = 0; i < conceptList.size(); i++) {
            ...
            for (int i1 = i+1; i1 < conceptList.size(); i1++) {
                ...
                if (c.equals(c1.getComplementNNF())){
                    LoggerManager.writeDebugLog("CLASH "+ OntologyRenderer.render(c) + " " +OntologyRenderer.render(c1), easyTableau.class);
                    return true;
                }
            }
        }
